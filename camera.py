...
class Camera:
    def __init__(self, name, pos, orient, active, lock=None):
        ...
        self.active = active
        self.lock = lock
        self.offset_amount = np.array([0, 0, 0])

        self.yaw = 0
        self.pitch = 0

        self.yaw_limits = [-80, 10]
...
##                self.offset_amount -= movement[2]
    ...
    def rotate(self, rotation):
        ...
        if self.pitch > np.deg2rad(self.yaw_limits[1]) and pitch_delta > 0:
            pitch_delta = 0
        elif self.pitch < np.deg2rad(self.yaw_limits[0]) and pitch_delta < 0:
            pitch_delta = 0
    ...
    def lock_to_target(self, target):
        self.lock = target
        rel_pos = self.orient[0] * self.offset_amount[0] + self.orient[1] * self.offset_amount[1] + self.orient[2] * self.offset_amount[2]
        self.set_pos(-self.lock.pos - rel_pos)
